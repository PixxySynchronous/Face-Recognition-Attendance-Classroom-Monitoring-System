#!/usr/bin/env python3
"""Sample a classroom video every 30 seconds, detect faces with the fine-tuned RetinaFace
ONNX model, crop each face tightly across a 15-frame window, and classify the resulting clip
with the fine-tuned 3D CNN R-18 model.

Pipeline:
1. Sample 15 consecutive frames every N seconds from the input video.
2. Run the fine-tuned RetinaFace detector on each sampled frame.
...
from utils.retinaface_detector import build_face_analysis_with_retinaface_detector
...
# Make project-root utils importable regardless of working directory
import sys as _sys
...
def default_face_model_name() -> str:
    return os.environ.get("INSIGHTFACE_MODEL_NAME", "buffalo_l")
...
class StudentIdentity:
    ...
    observations: int = 1
    last_bbox: tuple[int, int, int, int] | None = None   # centre of last known position

    def update(self, embedding: np.ndarray, bbox: tuple | None = None) -> None:
...
# Normalised centre-to-centre distance between two bboxes (0 = same, 1 = opposite corners).
# frame_w / frame_h used so the distance is scale-independent.
_POSITION_MATCH_RADIUS   = 0.12   # student must be within 12% of frame width/height to count as positional hit
_POSITION_MIN_SIMILARITY = 0.28   # lower embedding threshold when position already agrees
...
class StudentActivityPipeline:
    ...
    def detect_faces(self, frame: np.ndarray) -> list[dict]:
        ...
        faces = sahi_face_detect(self.face_detector.get, frame)
    ...
    def _bbox_centre_distance(
        ...
        cx_a = (a[0] + a[2]) / 2.0 / frame_w
        cy_a = (a[1] + a[3]) / 2.0 / frame_h
        cx_b = (b[0] + b[2]) / 2.0 / frame_w
        cy_b = (b[1] + b[3]) / 2.0 / frame_h
    ...
    def _resolve_student_identity(
        ...
        # ── Stage 1: position-first matching ──────────────────────────────────
        # Students stay in their seats between bursts.  Find the bank entry
        # whose last known bbox centre is closest to this track's centre.
        # If that candidate is also within the minimum embedding threshold, prefer
        # them over a potentially stronger but spatially distant embedding match.
        if current_bbox is not None:
            positional_candidates = [
                (self._bbox_centre_distance(current_bbox, identity.last_bbox, frame_w, frame_h), identity)
                for identity in student_bank
                if identity.last_bbox is not None
            ]
            if positional_candidates:
                positional_candidates.sort(key=lambda x: x[0])
                closest_dist, closest_identity = positional_candidates[0]
                closest_sim = float(np.dot(closest_identity.prototype, embedding))

                if closest_dist < _POSITION_MATCH_RADIUS and closest_sim >= _POSITION_MIN_SIMILARITY:
                    closest_identity.update(embedding, current_bbox)
                    return next_student_id, closest_identity.student_id, closest_sim

        # ── Stage 2: pure embedding matching (fallback) ───────────────────────
        ...
        if best_identity is not None and best_similarity >= self.identity_threshold:
            best_identity.update(embedding, current_bbox)
            return next_student_id, best_identity.student_id, best_similarity

        student_id = next_student_id
        new_identity = StudentIdentity(
            student_id=student_id, prototype=embedding.copy(), last_bbox=current_bbox
        )
        student_bank.append(new_identity)
        return next_student_id + 1, student_id, 0.0
    ...
    def _save_clip(self, frames: list[np.ndarray], clip_path: Path, fps: float) -> None:
        ...
        # Always export at a slow fixed rate so 24 frames → ~5 seconds regardless
        # of the source video fps (60fps source would otherwise give 0.4s clips).
        export_fps = 5.0
    ...
    def _process_window(
        ...
        for track in tracks:
            ...
            # Use the median bbox across the window as the track's representative position
            valid_boxes = [b for b in track.boxes if b is not None]
            track_bbox = valid_boxes[len(valid_boxes) // 2] if valid_boxes else None
            frame_h, frame_w = frames[0].shape[:2] if frames else (1080, 1920)
